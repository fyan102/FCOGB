...
class OrthogonalBoostingObjective(ObjectFunction):
    def __init__(self, data, target, predictions=None, loss=SquaredLoss, reg=1.0, rules=None, epsilon=1e-4, **kwargs):
        super().__init__(data, target, predictions, loss, reg, rules)
    ...
    def bound(self, ext):
        ...
        num_suf = abs(cumsum(g_q[::-1]))
        if len(self.rules) != 0:
            qs = self.fast_para_norms_prefix(ext) + self.epsilon
            qs_neg = self.fast_para_norms_prefix(ext[::-1]) + self.epsilon
        else:
...
def orthogonalize(mat_u, eps=1e-15):
    n = len(mat_u[0])
    mat_v = mat_u.T
    for i in range(n):
        prev_basis = mat_v[0:i]  # orthonormal basis before mat_v[i]
        coeff_vec = np.dot(prev_basis, mat_v[i].T)  # each entry is np.dot(mat_v[j], mat_v[i]) for all j < i
        # subtract projections of mat_v[i] onto already determined basis mat_v[0:i]
        mat_v[i] -= np.dot(coeff_vec, prev_basis).T
        if np.linalg.norm(mat_v[i]) < eps:
            mat_v[i][mat_v[i] < eps] = 0.  # set the small entries to 0
        else:
            mat_v[i] /= np.linalg.norm(mat_v[i])
    return mat_v.T
